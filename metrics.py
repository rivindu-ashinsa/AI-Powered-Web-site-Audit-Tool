"""Metrics module: computes factual, deterministic webpage metrics from parsed content."""
...
def compute_metrics(soup: BeautifulSoup, text: str, base_url: str) -> Dict[str, Any]:
    """Compute required audit metrics from a BeautifulSoup document and extracted text."""
    ...
    action_link_keywords = ("contact", "book", "buy", "start", "signup", "sign up", "get", "join", "try")
    action_links = []
    for link in soup.find_all("a", href=True):
        link_text = (link.get_text(" ", strip=True) or "").lower()
        if any(keyword in link_text for keyword in action_link_keywords):
    ...
    images_missing_alt_pct = round((missing_alt / image_count) * 100, 2) if image_count else 0.0
